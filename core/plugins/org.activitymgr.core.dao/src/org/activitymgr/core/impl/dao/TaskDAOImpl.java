...
public class TaskDAOImpl extends AbstractORMDAOImpl<Task> implements ITaskDAO {
	...
	public int getSubTasksCount(long parentTaskId) throws DAOException {
		try(PreparedStatement pStmt = tx().prepareStatement(
			...
			+ " from TASK as theTask"
			+ " left join TASK as subTask"
			+ " on subTask.tsk_path = concat(theTask.tsk_path, theTask.tsk_number)"
			+ " where theTask.tsk_id=? group by theTask.tsk_id")) {
	...
	public Task[] getSubTasks(String parentTaskPath, String filter) {
		...
		try(PreparedStatement pStmt = tx().prepareStatement(buf.toString())) {
	...
	public Task getFirstTaskMatching(String filter) {
		...
		StringWriter buf = new StringWriter();
		buf.append("select ").append(getColumnNamesRequestFragment("t"))
			.append(" from TASK as t")
			.append(" where t.tsk_name like ? or t.tsk_code like ?")
			.append(" order by t.tsk_path, t.tsk_number");
		try(PreparedStatement pStmt = tx().prepareStatement(buf.toString())) {
	...
	public long[] getContributedTaskIds(Collaborator contributor, Calendar fromDate,
		...
		StringBuilder request = new StringBuilder(
				"select distinct ctb_task, tsk_path, tsk_number "
				+ "from CONTRIBUTION, TASK where ctb_task=tsk_id");
