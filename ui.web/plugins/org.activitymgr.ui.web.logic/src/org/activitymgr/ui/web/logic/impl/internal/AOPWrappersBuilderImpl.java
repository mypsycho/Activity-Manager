...
final class AOPWrappersBuilderImpl implements IAOPWrappersBuilder {
	...
	@SuppressWarnings("unchecked")
	@Override
	public <T> T buildLogicWrapperForView(final T wrapped, Class<T> interfaceToWrapp) {
		return (T) Proxy.newProxyInstance(
			wrapped.getClass().getClassLoader(),
			new Class<?>[] { interfaceToWrapp }, 
			(proxy, method, args) -> {
				...
				try {
					...
					// Prepare the call
					Object result = null;
					Boolean notificationsDisabled = viewNotificationsDisabled.get();
					...
					if (method.getReturnType().equals(void.class)
							&& Boolean.TRUE.equals(notificationsDisabled)) {
